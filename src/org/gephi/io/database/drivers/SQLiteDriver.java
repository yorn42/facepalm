...
public class SQLiteDriver implements SQLDriver {
	...
	public SQLiteDriver(SQLiteHelper todb) {
		this.todb = todb;
		try {
			// load the sqlite-JDBC driver using the current class loader
			Class.forName("org.sqlite.JDBC");
		} catch (ClassNotFoundException ex) {
			Logger.getLogger(SQLiteDriver.class.getName()).log(Level.SEVERE,
					null, ex);
		}
	}
	...
	public Connection getConnection(String connectionUrl, String username,
		...
		// as we are in memory with sqlite, we must use our previous connection.
		return todb.getConnection();
	}
